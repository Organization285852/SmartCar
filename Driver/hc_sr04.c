...
{	 
  ...
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_6|GPIO_Pin_7;  //PB6 清除之前设置  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PB6 输入  
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOB,GPIO_Pin_6|GPIO_Pin_7);			//PB6 下拉
  ...
  TIM_ICInit(TIM4, &TIM4_ICInitStructure);
		...
		//初始化TIM4输入捕获参数
	TIM4_ICInitStructure.TIM_Channel = TIM_Channel_2; //CC2S=01 	选择输入端 IC2映射到TI2上
  TIM4_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;	//上升沿捕获
...
{ 
    ...
    {	
			...
			{
				TIM4CH1_CAPTURE_F = TIM4->CNT;//记录定时器值
				TIM4CH1_CAPTURE_STA=0X40;		//标记捕获到了上升沿
		   	TIM_OC1PolarityConfig(TIM4,TIM_ICPolarity_Falling);		//CC1P=1 设置为下降沿捕获
			}		    
		}	
    ...
    {	
			...
			{
				TIM4CH2_CAPTURE_F = TIM4->CNT;//记录定时器值
...
void Init_hc_sr04(void)
{
 ...
 //	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);  //使能GPIOA时钟
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_13|GPIO_Pin_8;  //PB5 清除之前设置  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; //PB5 输入
  
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	//红外避障
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_9|GPIO_Pin_14;  //PB6 清除之前设置  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PB6 输入  
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	
	GPIO_ResetBits(GPIOB,GPIO_Pin_13|GPIO_Pin_8);						 //PB5 
	
	TIM4_Cap_Init(0XFFFF,72-1);  //以1Mhz 的频率计数 
}
